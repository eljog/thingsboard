...
export function DeviceCardController(types) {
  ...
  vm.isAssignedToCustomer = function() {
    if (
      vm.item &&
      vm.item.customerId &&
      vm.parentCtl.devicesScope === "tenant" &&
      vm.item.customerId.id != vm.types.id.nullUid &&
      !vm.item.assignedCustomer.isPublic
    ) {
      return true;
    }
    return false;
...
export function DeviceController(
  ...
  vm.types = types;
  vm.render = renderTopology;
  vm.topologyMessage = "Click Render to render a topology";
  vm.topologyTree = {};
  ...
  vm.deviceGridConfig = {
    ...
    addItemText: function() {
      return $translate.instant("device.add-device-text");
    },
    noItemsText: function() {
      return $translate.instant("device.no-devices-text");
    },
    itemDetailsText: function() {
      return $translate.instant("device.device-details");
    },
  ...
  function renderTopology(full) {
    // create an array with nodes
    vm.grid.triggerResize();

    vm.topologyMessage = "Rendering...";

    if (topologyNetwork === null) {
      vm.topologyMessage = "Click Render button to render the topology";
    } else {
    ...
    deviceService
      .getDeviceTopology(
        vm.grid.detailsConfig.currentItem.name,
        full,
        true,
        null
      )
      .then(
        function success(topology) {
          data = topology;
          $log.info("Topology fetch success: \n" + angular.toJson(data));
          vm.topologyMessage = "Rendered!";
          prepareVisNetwork(data);
        },
        function fail() {
          $log.error("Topology fetch failed:");
          vm.topologyMessage =
            "Topology does not exist for: " +
            vm.grid.detailsConfig.currentItem.name;
        }
      );

    renderTopologyListView(full);
  }

  function renderTopologyListView(full) {
    deviceService
      .getDeviceTopologyTree(
        vm.grid.detailsConfig.currentItem.name,
        full,
        true,
        null
      ...
      .then(
        function success(topology) {
          vm.topologyTree = topology;
          $log.info(
            "Topology list fetch success:: \n" + angular.toJson(vm.topologyTree)
          );
        },
        function fail() {
          $log.error("Topology list fetch failed:");
        }
      );
  }

  function prepareVisNetwork(data) {
    var deviceStatusToColor = {
      UNKNOWN: "black",
      TLS_FAILUER: "orange",
      DEFAULT_STATUS: "yellow",
      DEVICE_CONFIGURED: "green"
    };

    var linkSpeedToColor = function(uplinkSpeed, downlinkSpeed) {
      var speed = Math.min(uplinkSpeed, downlinkSpeed);
      if (speed / 1000 < 10) {
        return "red";
    ...
    if (data !== null) {
      $log.info(
        "Rendering topology for device named: " +
          angular.toJson(vm.grid.detailsConfig.currentItem.name)
      );
      var visNodes = [];
      var visEdges = [];
      data.nodes.forEach(node => {
        ...
        visNode.color = {
          background: deviceStatusToColor[node.deviceStatus],
          border: "purple"
        };
        ...
        visNode.icon = {
          color: deviceStatusToColor[node.deviceStatus]
        };

        visNode.group = node.deviceType;
      ...
      data.edges.forEach(edge => {
        var visEdge = {};
        visEdge.from = edge.fromId;
        visEdge.to = edge.toId;
        visEdge.label =
          "↑" + edge.uplinkSpeed / 1000.0 + "\n↓" + edge.downlinkSpeed / 1000.0;

        var linkColor = linkSpeedToColor(edge.uplinkSpeed, edge.downlinkSpeed);
        visEdge.color = {
          color: linkColor,
          hover: linkColor,
          highlight: linkColor
      ...
      var container = $document[0].getElementById("topology");
      var visData = {
  ...
  function initController() {
    ...
    if (customerId) {
      ...
      customerService
        .getShortCustomerInfo(customerId)
        .then(function success(info) {
          if (info.isPublic) {
            vm.customerDevicesTitle = $translate.instant(
              "customer.public-devices"
            );
          }
        });
    }
    ...
    if (vm.devicesScope === "tenant") {
      ...
      deviceActionsList.push({
        ...
        name: function() {
          return $translate.instant("action.share");
        },
        details: function() {
          return $translate.instant("device.make-public");
        },
        icon: "share",
        isEnabled: function(device) {
          return (
            device &&
            (!device.customerId || device.customerId.id === types.id.nullUid)
          );
        }
      });
      ...
      deviceActionsList.push({
        ...
        details: function() {
          return $translate.instant("device.assign-to-customer");
        },
        icon: "assignment_ind",
        isEnabled: function(device) {
          return (
      ...
      deviceActionsList.push({
        ...
        details: function() {
          return $translate.instant("device.unassign-from-customer");
        },
        ...
        isEnabled: function(device) {
          return (
            device &&
            device.customerId &&
            device.customerId.id !== types.id.nullUid &&
            !device.assignedCustomer.isPublic
          );
        }
      });
      ...
      deviceActionsList.push({
        ...
        name: function() {
          return $translate.instant("action.make-private");
        },
        details: function() {
          return $translate.instant("device.make-private");
        },
        icon: "reply",
        isEnabled: function(device) {
          return (
      ...
      deviceActionsList.push({
        ...
        name: function() {
          return $translate.instant("device.credentials");
        },
        details: function() {
          return $translate.instant("device.manage-credentials");
        },
        icon: "security"
      });
      ...
      deviceActionsList.push({
        ...
        details: function() {
          return $translate.instant("device.delete");
        },
        icon: "delete"
      });
      ...
      deviceGroupActionsList.push({
        ...
        name: function() {
          return $translate.instant("device.assign-devices");
        },
        details: function(selectedCount) {
          return $translate.instant(
            "device.assign-devices-text",
            { count: selectedCount },
            "messageformat"
          );
        },
      ...
      deviceGroupActionsList.push({
        ...
        name: function() {
          return $translate.instant("device.delete-devices");
        },
        details: deleteDevicesActionTitle,
        icon: "delete"
      });
    } else if (
      vm.devicesScope === "customer" ||
      vm.devicesScope === "customer_user"
    ) {
      fetchDevicesFunction = function(pageLink, deviceType) {
      ...
      if (vm.devicesScope === "customer") {
        ...
        deviceGroupActionsList.push({
          ...
          name: function() {
            return $translate.instant("device.unassign-devices");
          },
          details: function(selectedCount) {
            return $translate.instant(
              "device.unassign-devices-action-title",
              { count: selectedCount },
              "messageformat"
            );
          },
        ...
        vm.deviceGridConfig.addItemAction = {
          ...
          details: function() {
            return $translate.instant("device.assign-new-device");
          },
          icon: "add"
        };
        ...
        deviceActionsList.push({
          ...
          details: function() {
            return $translate.instant("device.view-credentials");
          },
          icon: "security"
        });
  ...
  function deleteDeviceTitle(device) {
    return $translate.instant("device.delete-device-title", {
  ...
  function deleteDevicesActionTitle(selectedCount) {
    return $translate.instant(
      "device.delete-devices-action-title",
      { count: selectedCount },
      "messageformat"
    );
  }
  ...
  function addDevicesToCustomer($event) {
    ...
    var pageSize = 10;
    deviceService
      .getTenantDevices({ limit: pageSize, textSearch: "" }, false)
      .then(
  ...
  function unassignFromCustomer($event, device, isPublic) {
    ...
    var label;
    if (isPublic) {
      title = $translate.instant("device.make-private-device-title", {
        deviceName: device.name
      });
      content = $translate.instant("device.make-private-device-text");
      label = $translate.instant("device.make-private");
    } else {
      title = $translate.instant("device.unassign-device-title", {
        deviceName: device.name
      });
    ...
    $mdDialog.show(confirm).then(function() {
      deviceService
        .unassignDeviceFromCustomer(device.id.id)
  ...
  function unassignDevicesFromCustomer($event, items) {
    var confirm = $mdDialog
      ...
      .title(
        $translate.instant(
          "device.unassign-devices-title",
          { count: items.selectedCount },
          "messageformat"
        )
      )
  ...
  function makePublic($event, device) {
    ...
    var confirm = $mdDialog
      ...
      .targetEvent($event)
      .title(
        $translate.instant("device.make-public-device-title", {
          deviceName: device.name
        })
      )
