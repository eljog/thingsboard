...
export default angular
  .module("thingsboard.api.device", [thingsboardTypes])
...
function DeviceService(
  ...
  var service = {
    ...
    getDeviceTopology: getDeviceTopology,
    getDeviceTopologyTree: getDeviceTopologyTree
  };
  ...
  function getCustomerDevices(
    ...
    var deferred = $q.defer();
    var url =
      "/api/customer/" + customerId + "/devices?limit=" + pageLink.limit;
    if (angular.isDefined(pageLink.textSearch)) {
      url += "&textSearch=" + pageLink.textSearch;
    }
    if (angular.isDefined(pageLink.idOffset)) {
    ...
    $http.get(url, config).then(
      function success(response) {
        if (applyCustomersInfo) {
          customerService
            .applyAssignedCustomerInfo(response.data.data, customerId)
            .then(
              function success(data) {
                response.data.data = data;
                deferred.resolve(response.data);
  ...
  function saveDeviceCredentials(deviceCredentials) {
    ...
    $http.post(url, deviceCredentials).then(
      function success(response) {
        deferred.resolve(response.data);
      },
      function fail() {
  ...
  function subscribeForDeviceAttributes(deviceId, attributeScope) {
    return attributeService.subscribeForEntityAttributes(
      types.entityType.device,
      deviceId,
      attributeScope
    );
  ...
  function deleteDeviceAttributes(deviceId, attributeScope, attributes) {
    return attributeService.deleteEntityAttributes(
      types.entityType.device,
      deviceId,
      attributeScope,
      attributes
  ...
  function findByQuery(query, ignoreErrors, config) {
    ...
    config = Object.assign(config, { ignoreErrors: ignoreErrors });
    $http.post(url, query, config).then(
      function success(response) {
        deferred.resolve(response.data);
      },
      function fail() {
